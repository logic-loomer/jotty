...
Requires macOS 26.0+ (the default on-device AI extraction uses Apple Intelligence / FoundationModels, which is macOS 26-only) and Xcode 16+ (Xcode 26 recommended). The project file is generated by [XcodeGen](https://github.com/yonaskolb/XcodeGen) from `project.yml`, so install that first:
...
Re-run `xcodegen generate` whenever you add or remove source files (the `.xcodeproj` is generated, not hand-edited).
...
Contributions are welcome. Jotty is test-first: every behaviour change ships with a test, and the full `JottyTests` suite runs on every push and PR (see [Testing](#testing)).

- **[CONTRIBUTING.md](CONTRIBUTING.md)** - how to build, test, and open a PR, plus the TDD and review conventions this repo follows.
- **[SECURITY.md](SECURITY.md)** - how to report a vulnerability privately (Jotty handles API keys and Keychain secrets, so please do not file security issues in the public tracker).
- **[CODE_OF_CONDUCT.md](CODE_OF_CONDUCT.md)** - the Contributor Covenant we ask everyone to follow.

Bugs and feature ideas go through the [issue templates](.github/ISSUE_TEMPLATE); pull requests use the [PR template](.github/pull_request_template.md).
